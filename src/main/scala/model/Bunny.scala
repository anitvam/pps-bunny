...
sealed trait Bunny {
  ...
  override def toString
    ...
    .map(g =>
      "\t" + g._1 + ": " + g._2.getVisibleTrait.toString.toLowerCase + " (" + g._2.getLetters + ")")
    .reduce(_ + "\n" + _)
    .replace("_", " ") + "\n"
...
class ChildBunny(override val genotype: CompletedGenotype,
...
class FirstBunny(genotype: CompletedGenotype)
    extends ChildBunny(genotype, Option.empty, Option.empty)
  ...
  def generateBaseFirstBunny: FirstBunny = new FirstBunny(
    CompletedGenotype(
      Genes.values.unsorted
        .map(gk => (gk, Gene(gk, StandardAllele(gk.base), StandardAllele(gk.base))))
        .toMap))

  /** @return a FirstBunny with a random allele for each gene */
  def generateRandomFirstBunny: FirstBunny = new FirstBunny(
    CompletedGenotype(
