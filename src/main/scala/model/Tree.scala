 ...
 *  @tparam A        the type of the element
 */
case class Node[A](override val elem: A, momTree: BinaryTree[A], dadTree: BinaryTree[A])
...
object Tree {
  ...
  def generateTree(generations: Int, bunny: Bunny): BinaryTree[Bunny] =
    if (generations == 1 || bunny.mom.isEmpty) Leaf(bunny)
              ...
              generateTree(generations - 1, bunny.mom.get),
              generateTree(generations - 1, bunny.dad.get))
