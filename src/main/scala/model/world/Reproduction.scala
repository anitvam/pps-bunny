...
object Reproduction {
  ...
  def generateAllChildren(bunnies: Population, mutations: Mutations = List()): Population = {
    ...
    val (coupleWithMutations, coupleWithoutMutations) =
      Random.shuffle(couples).splitAt((couples.length / 2) + 1)
  ...
  def generateChildren(mom: Bunny, dad: Bunny, mutations: Mutations = List()): Population = {
    ...
    Genes.values.foreach { gk =>
      ...
      val genesOfReproduction: List[Gene] = List(
        Gene(gk, grandmaMomAllele, grandmaDadAllele),
      ...
      childrenGenotypes = (for (i <- 0 until CHILDREN_FOR_EACH_COUPLE)
        yield childrenGenotypes(i) + shuffledGenes(i)).toList
      ...
      mutations.filter(_.geneKind == gk).foreach { m =>
        childrenGenotypes = childrenGenotypes(CHILDREN_FOR_EACH_COUPLE - 1) +
          Gene(m.geneKind,
               ...
               JustMutatedAllele(gk.mutated)) :: childrenGenotypes.take(
            CHILDREN_FOR_EACH_COUPLE - 1)
      }

    }
    childrenGenotypes.map(cg =>
      new ChildBunny(CompletedGenotype(cg.genes), Option(mom), Option(dad)))
  }
