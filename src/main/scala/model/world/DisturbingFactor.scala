...
sealed trait Factor {
   ...
   * @param bunnies the actual Population
   * @param climate the actual Climate
   * @return the remaining Population */
...
object Factor {
  ...
  abstract class BasicFactor extends Factor {
    override def applyDamage(bunnies: Population, climate: Climate): Population = killPercentageBunnies(bunnies, normalDamage)
  }
  ...
  sealed trait FoodFactor extends Factor {
     ...
     * @return true if ToughFood is active */
    ...
    abstract override def applyDamage(bunnies: Population, climate: Climate): Population = {
      ...
      (isHighFood, isLimitedFood, isToughFood) match {
        ...
        case (true, false, true) =>
          super.applyDamage(bunniesSplitByHighFoodGene._1, climate)
          killPercentageBunnies(filterBunniesWithAlleles(bunnies, highFoodDamagedGene.mutated, toughFoodDamagedGene.base), lowDamage)
        ...
        case (true, true, true) =>
          super.applyDamage(bunniesSplitByHighFoodGene._1, climate)
          super.applyDamage(filterBunniesWithAlleles(bunnies, highFoodDamagedGene.mutated, toughFoodDamagedGene.base), climate)
          killPercentageBunnies(filterBunniesWithAlleles(bunnies, highFoodDamagedGene.mutated, toughFoodDamagedGene.mutated), lowDamage)
      }
      bunnies
    }
  ...
  case class FoodFactorImpl(override var isHighFood: Boolean,
                   override var isLimitedFood: Boolean,
                   override var isToughFood: Boolean) extends BasicFactor with FoodFactor

  abstract class ClimateFactor extends Factor {

    /** Action performed on a Population of Bunnies on summer
     * @param bunnies the population
     * @return the population updated */
  ...
  sealed trait PredatorFactor extends Factor {
    ...
    def lowDamage: Double

    /** @return the high damage that is applied with this Factor */
    def highDamage: Double
  }

  case class UnfriendlyClimate(override val normalDamage: Double = UNFRIENDLY_CLIMATE_DAMAGE,
                               afflictedGene: GeneKind = Genes.FUR_LENGTH) extends ClimateFactor with FactorWithOneGene {

    protected override def summerAction(bunnies: Population): Population = {
    ...
    protected override def winterAction(bunnies: Population): Population = {
      val splitBunnies = Bunny.splitBunniesByGene(afflictedGene, bunnies)
      killPercentageBunnies(splitBunnies._1, normalDamage) ++ splitBunnies._2
    }
  }

  case class Wolves(normalDamage: Double = WOLF_MEDIUM_DAMAGE,
                    lowDamage: Double = WOLF_LOW_DAMAGE,
                    highDamage: Double = WOLF_HIGH_DAMAGE) extends ClimateFactor with PredatorFactor {

    private val firstGeneAffected: GeneKind = Genes.FUR_COLOR
    private val secondGeneAffected: GeneKind = Genes.EARS

    override def summerAction(bunnies: Population): Population = manageKillBunnies(bunnies, filterBunniesWithAlleles(bunnies, firstGeneAffected.mutated, secondGeneAffected.base), filterBunniesWithAlleles(bunnies, firstGeneAffected.base, secondGeneAffected.mutated))

    override def winterAction(bunnies: Population): Population = manageKillBunnies(bunnies, filterBunniesWithAlleles(bunnies, firstGeneAffected.base, secondGeneAffected.base), filterBunniesWithAlleles(bunnies, firstGeneAffected.mutated, secondGeneAffected.mutated))

    private def manageKillBunnies(bunnies: Population, bunniesWithLowDamage: Population, bunniesWithHighDamage: Population): Population = {
...
object FactorsUtils {
   ...
   * @param bunnies a Population of bunnies
   * @param percentage a percentage of bunnies
   ...
   * @param bunnies the bunny population
   * @param allele1 the first allele filtered on the population
   * @param allele2 the second allele filtered in population
   * @return Population the population with the specified AlleleKinds together */
  def filterBunniesWithAlleles(bunnies: Population, allele1: AlleleKind, allele2: AlleleKind): Population = bunnies filter { bunny =>
