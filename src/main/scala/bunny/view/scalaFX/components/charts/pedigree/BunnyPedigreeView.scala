...
import bunny.view.scalaFX.ScalaFXConstants.GenealogicalTree.{
  BUNNY_ALLELE_PADDING, FONT_INFO_PERCENT, TREE_INFO_PROPORTION
}
import bunny.view.scalaFX.components.charts.pedigree.PedigreeChart.{ bunnyIconSize, spacingRegion }
import bunny.view.scalaFX.utilities.Direction.Right
import bunny.view.scalaFX.utilities.{ BunnyImageUtils, Direction, ImageType }

/**
 * Represents the bunny.view on the Bunny in a tree.
 */
trait BunnyPedigreeView {
  ...
  /** Pane with actual bunny.view of the bunny */
  val pane: Pane
}
