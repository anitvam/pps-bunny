...
import view.scalaFX.components.charts.LineChartComponentFactory.{
...
object PopulationChartDataType {
  ...
  case class MutationsChartSeries(var mutationMap: Map[AlleleKind, ChartSeries] = Map()) {
    ...
    Alleles.values.foreach { ak =>
      mutationMap =
        mutationMap + (ak -> ChartSeries(SeriesData(), createEmptySeries(ak.prettyName)))
    }
    ...
    def + (ak: AlleleKind, p: Point): Unit = mutationMap(ak) + p
  }

  /** Ad hoc ChartPoint extractor */
  object ChartPoint {

    def unapply(arg: ChartPoint): Option[((Double, Int), Boolean)] =
      Some(((arg.point.x, arg.point.y), arg.isTruePoint))
...
object PopulationChart {
  ...
  def updateChart(generationPhase: GenerationPhase, population: Population): Unit = {
    ...
    Alleles.values.foreach(ak =>
      mutations + (ak, (generationPhase,
                        population.count(_.genotype.phenotype.values.toSet.contains(ak)))))
...
object ChartConverters {
  ...
  implicit def fromTupleToChartPoint(t: (Double, Int, Boolean)): ChartPoint =
    ChartPoint(Point(t._1, t._2), t._3)
...
object LineChartComponentFactory {
   ...
   */
  def createNumberAxis(name: String,
                       lowerBound: Int,
                       upperBound: Int,
                       tickUnit: Int): NumberAxis = {
   ...
   */
  def createSeries(name: String, s: SeriesData): XYSeries = XYChart.Series(
    name = name,
   ...
   */
  def createLineChart(xAxis: NumberAxis,
                      yAxis: NumberAxis,
                      chartHeight: Double,
                      chartWidth: Double,
